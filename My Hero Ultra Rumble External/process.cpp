...
uintptr_t GNames = 0x5FC2F00;
uintptr_t GObjects = 0x5FFF250;
...
namespace util {
    ...
    bool init() {
        ...
        procID = getProcID("MHUR.exe");
        if (procID) {

            modBase = GetModuleBaseAddress(procID, "MHUR.exe");
            hProc = OpenProcess(PROCESS_ALL_ACCESS, NULL, procID);

            GNames += modBase;
            GNames = util::Read<uintptr_t>(GNames, sizeof(GNames));
            GObjects += modBase;
            targetWindow = FindWindow("UnrealWindow", "MHUR  ");
