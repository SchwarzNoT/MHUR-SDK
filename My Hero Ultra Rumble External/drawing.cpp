...
ImFont* Font;
std::vector<AimSettings> aimSettings(1);
	...
	ImGui::Begin("Menu", nullptr, ImGuiWindowFlags_NoDecoration);
	{
		...
		ImGui::BeginChild("##AimSettings", ImVec2((4 * menuDimensions.x / 9), menuDimensions.y - 30.f), true, ImGuiWindowFlags_NoDecoration);
		{
			...
			if (bSpeedHack) {
					...
					ImGui::SliderFloat("Speed", &speedValue, 0.f, 360.f);

				}

			

			ImGui::Checkbox("Giant Mode", &giantMode);
			if (giantMode) {

				ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 20.f);

				ImGui::SliderFloat("Size", &GiantScale, 1.f, 360.f);
			}
			
			ImGui::Checkbox("Projectile TP", &bulletTP);
